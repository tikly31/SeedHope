...
public class Userservice {
    ...
    public User register(User user) {
        ...
        // Step 3: Validate the contact number (must be 11 digits and only numeric)
    ...
    // Helper method to validate contact number (must be exactly 11 digits and only numbers)
    private boolean isValidContactNumber(String contactno) {
        // Check if contact number is not null, has exactly 11 characters, and contains only digits
        return contactno != null && contactno.length() == 11 && contactno.matches("\\d{11}");
    }
